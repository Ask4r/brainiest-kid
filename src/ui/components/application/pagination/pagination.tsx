...
import { ButtonGroup, ButtonGroupItem } from "@/ui/components/base/button-group/button-group";
...
function PaginationItem({ value, rounded, isCurrent }: { value: number, rounded?: boolean, isCurrent: boolean }) {
    return (
        <Pagination.Item
            value={value}
            isCurrent={isCurrent}
...
function MobilePagination({ page = 1, total = 10, className, onPageChange }: MobilePaginationProps) {
    return (
...
export function PaginationPageDefault({ rounded, page = 1, total = 10, className, ...props }: PaginationProps) {
            ...
            <div className="hidden flex-1 justify-start md:flex">
                <Pagination.PrevTrigger asChild>
                    <Button iconLeading={ArrowLeft} color="link-gray" size="sm">
                        {isDesktop ? "Предыдущая" : undefined}{" "}
                    </Button>
                ...
                {({ pages, currentPage, total }) => (
                    <>
                        ...
                        <div className="flex justify-center text-sm whitespace-pre text-fg-secondary md:hidden">
                            Страница <span className="font-medium">{currentPage}</span> из <span className="font-medium">{total}</span>
                        </div>
                    </>
                )}
            </Pagination.Context>
...
export function PaginationPageMinimalCenter({ rounded, page = 1, total = 10, className, ...props }: PaginationProps) {
    const isDesktop = useBreakpoint("md");
...
export function PaginationCardDefault({ rounded, page = 1, total = 10, ...props }: PaginationProps) {
    const isDesktop = useBreakpoint("md");
...
export function PaginationCardMinimal({ page = 1, total = 10, align = "left", onPageChange, className }: PaginationCardMinimalProps) {
    return (
...
export function PaginationButtonGroup({ align = "left", page = 1, total = 10, ...props }: PaginationButtonGroupProps) {
    const isDesktop = useBreakpoint("md");
