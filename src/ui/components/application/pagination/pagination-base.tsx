...
function PaginationRoot({ total, siblingCount = 1, page, onPageChange, children, style, className }: PaginationRootProps) {
...
function PaginationItem({ value, isCurrent, children, style, className, ariaLabel, asChild = false }: PaginationItemProps) {
  const context = useContext(PaginationContext);
  if (!context) {
    throw new Error("Pagination components must be used within a <Pagination.Root />");
  }
