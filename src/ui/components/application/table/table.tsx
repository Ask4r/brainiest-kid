...
import { Tooltip, TooltipTrigger } from "@/ui/components/base/tooltip/tooltip";
import { cx } from "@/ui/utils/cx";

export function TableRowActionsDropdown() {
...
function TableCardRoot({ children, className, size = "md", ...props }: HTMLAttributes<HTMLDivElement> & { size?: "sm" | "md" }) {
  return (
...
function TableRoot({ className, size = "md", ...props }: TableRootProps) {
...
function TableHeader<T extends object>({ columns, children, bordered = true, className, size: sizeProp, ...props }: TableHeaderProps<T>) {
  const context = useContext(TableContext);
...
function TableHead({ className, tooltip, label, children, ...props }: TableHeadProps) {
...
function TableRow<T extends object>({ columns, children, className, highlightSelectedRow = true, size: sizeProp, ...props }: TableRowProps<T>) {
  const context = useContext(TableContext);
  const { selectionBehavior } = useTableOptions();
...
function TableCell({ className, children, size: sizeProp, ...props }: TableCellProps) {
  const context = useContext(TableContext);
  const { selectionBehavior } = useTableOptions();
