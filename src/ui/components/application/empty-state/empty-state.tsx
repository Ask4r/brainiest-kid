...
function FeaturedIcon({ color = "gray", theme = "modern", icon = SearchLg, size, ...props }: ComponentPropsWithRef<typeof FeaturedIconbase>) {
...
function AvatarRadius({ avatars = [], ...props }: AvatarRadiusProps) {
  return (
    <div
      aria-hidden="true"
      {...props}
...
function AvatarRow({ avatars = [], children, ...props }: AvatarRowProps) {
  const { size: rootSize } = useContext(RootContext);
...
function AvatarGrid({ avatars = [], ...props }: AvatarGridProps) {
  const { size: rootSize } = useContext(RootContext);
  const sizeKey = rootSize || "md";
