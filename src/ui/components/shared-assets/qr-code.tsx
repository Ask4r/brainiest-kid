...
function QRCodeFrameHandle({ className, ...props }: HTMLAttributes<HTMLDivElement>) {
    return <div {...props} className={cx("size-3 rounded-tl border-t-2 border-l-2 border-brand_alt", className)} />;
}

export function GradientScan({ className, ...props }: HTMLAttributes<HTMLDivElement>) {
    return (
        <div
            {...props}
...
export function QRCode({ size = "md", value, options, className }: QRCodeProps) {
