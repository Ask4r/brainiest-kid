...
export function Round1Player() {
  ...
  const [lastSubmitAnswerIdx, setLastSubmitAnswerIdx] = useState<number | undefined>(undefined);
  ...
  const handleAnswerClick = (idx: number) => {
    if (!outOfTime) {
      playerActions.round1SubmitAnswer(playerId, idx, idx === currentQuestion.correctIdx);
      setLastSubmitAnswerIdx(idx);
    }
  };
  ...
  const isButtonDisabled = useCallback((idx: number) => {
    ...
    if (isShowQuestion && lastSubmitAnswerIdx !== undefined) {
      return idx !== lastSubmitAnswerIdx;
    }
    return outOfTime;
  }, [isButtonGreen, outOfTime, isShowQuestion, lastSubmitAnswerIdx]);
