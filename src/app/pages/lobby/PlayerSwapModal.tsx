...
import { Dialog, Modal, ModalOverlay } from "@/ui/components/application/modals/modal";
        ...
        <Dialog aria-labelledby="dialog-heading">
          {({ close }) => (
            <section className="relative max-h-[inherit] w-full overflow-y-auto outline-hidden" role="dialog" tabIndex={-1}>
              <ButtonUtility color="tertiary" icon={X} className="absolute top-3 right-3 sm:top-4 sm:right-4" onClick={close} />
              <div className="flex flex-col gap-4 px-4 pt-5 sm:px-6 sm:pt-6">
                <FeaturedIcon color="gray" theme="modern" icon={Repeat01} />
                <div className="z-10 flex flex-col gap-0.5">
                  <h3 id="dialog-heading" className="text-md font-semibold text-primary">
              ...
              <div className="z-10 flex flex-1 flex-col-reverse gap-3 p-4 pt-6 *:grow sm:grid sm:grid-cols-2 sm:px-6 sm:pt-8 sm:pb-6">
                <Button color="primary" size="md" isDisabled={playerId === undefined} onClick={() => {
                  handleConfirmClick();
                  close();
                }}>Подтвердить</Button>
                <Button color="secondary" size="md" onClick={close}>Отмена</Button>
              </div>
            </section>
          )}
        </Dialog>
